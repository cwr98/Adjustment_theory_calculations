...
%solve takes a equation and solves it 

f=Cval-((x-2)/4)*Cval; %true PDF with C 
%f(x)=3/4 - x/8 = result of lin
%now we integrate it
%x=value we move around during int
%2 is starting, always
%b end point until all f(x) are summed up
F = int(f,x,2,b); %finding the distribution function is depent on b
%F(b)=-((b - 2)*(b - 10))/16 
%x = result of random var put the 2nd x to tell comp to run x from 2-6
